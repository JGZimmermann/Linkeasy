...
class ProfileAvatar
{
  ...
  public function path(): string
  {
    if ($this->model->photo_url) {
      return $this->baseDir() . $this->model->photo_url;
    }
  ...
  public function update(array $image): void
  {
    ...
    if (!empty($this->getTmpFilePath())) {
      $this->removeOldImage();
      $this->model->update(['photo_url' => $this->getFileName()]);
  ...
  {
    if ($this->model->photo_url) {
      $path = Constants::rootPath()->join('public' . $this->baseDir())->join($this->model->photo_url);
